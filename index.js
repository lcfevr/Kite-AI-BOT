...
async function main() {
    ...
    // Shuffle wallets and proxies together
    const combined = wallets.map((wallet, index) => ({ wallet, proxy: proxyList[index] }));
    shuffleArray(combined);
    
    const shuffledWallets = combined.map(item => item.wallet);
    const shuffledProxies = combined.map(item => item.proxy);
    
    console.log(`${chalk.cyan('📊 Loaded:')} ${chalk.green(shuffledWallets.length)} wallets and ${chalk.green(shuffledProxies.length)} proxies\n`);
    
    // Create instances for each wallet with unique session IDs
    const instances = shuffledWallets.map((wallet, index) => 
        new KiteAIAutomation(wallet, shuffledProxies, index + 1)
    );
